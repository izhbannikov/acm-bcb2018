...
for(jj in 1:nexp) {
    ...
    imp.data <- missForest(xmis=incomplete.data, maxiter = 2)$ximp
...
write.table(x=spm.results, file=paste(pdir, "simulated/", "spm.results.missforest.", N, ".", nexp, ".tsv", sep=""), row.names = FALSE, col.names = TRUE)
write.table(x=spm.results.imputed, file=paste(pdir, "simulated/", "spm.results.imputed.missforest.", N, ".", nexp, ".tsv", sep=""), row.names = FALSE, col.names = TRUE)
...
pdf(file=paste(pdir, "/true_vs_imputed_missForest.", nd, ".pdf", sep=""), width = 16, height = 12)
    multiplot(p1, p2, p3, p4, p5, p6, p7, cols = 3, rows=3, title = "Parameters estimated from true against imputed data with method Random Forest")
dev.off()
